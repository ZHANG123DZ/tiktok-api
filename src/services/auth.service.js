...
const { RefreshToken, User, EmailVerification } = require('@/models/index');
...
class authService {
  ...
  async sendCode(data) {
    ...
    const code = await EmailVerification.create(data);
    return code.dataValues;
  }

  async verifyEmail(data) {
    const auth = await EmailVerification.findOne({
      where: {
        email: data.email,
        otp_code: data.code,
        expired_at: {
